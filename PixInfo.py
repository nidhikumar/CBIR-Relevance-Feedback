...
class PixInfo:
    ...
    def refreshPics(self, folderPath):
        ...
        image_files = sorted(glob.glob(self.picPath + '/*.jpg'), key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    ...
    def getFileInt(self, fn):
        i = fn.rfind('/')  # Find the last '/' character
        j = fn.rfind('.')  # Find the last '.' character
        if i != -1 and j != -1:  # Check if '/' and '.' are found in the string
            return int(fn[i + 1:j])  # Extract and convert the integer part
        else:
            # Handle cases where '/' or '.' is not found
            return -1
